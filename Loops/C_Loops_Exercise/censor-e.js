...
//The function should return the a new version of string where all characters that are 'e's are replaced with '*'s.

let censorE = function (str) {
    let nustring = "";
    for (let i = 0; i < str.length; i++) {
         let char = str[i]; //console.log(str[i]) s, p, e, e, d, y
         if (char === "e") {
            nustring += "*";
         } else {
             nustring += char;
